...
float total_bytes_written = 0;   // 총 쓰기 바이트 수
time_t last_throughput_time = 0; // 마지막 throughput 출력 시간
    ...
    while (1)
    {
        for (i = 1; i <= n->nr_pollers; i++)
        {
            ...
            case NVME_CMD_WRITE:
                ...
                // 총 쓰기 바이트 수 증가
                total_bytes_written += req->nlb * ssd->sp.secs_per_pg * ssd->sp.secsz;
        ...
        if (current_time - last_throughput_time >= 1)
        {                                                               // 1초마다 출력
            float throughput = total_bytes_written / (1024.0 * 1024.0); // MB 단위로 변환
            printf("Throughput: %f MB/s\r\n", throughput);
            last_throughput_time = current_time;
            total_bytes_written = 0; // 바이트 수 초기화
        }
    }
